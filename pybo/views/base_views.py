...
def category(request, question_category):
    # 입력 파라미터
    c = Category.objects.get(title=question_category)
    uestion = c.question_set.all()
    page = request.GET.get('page', '1')  # 페이지
    kw = request.GET.get('kw', '')  # 검색어
    so = request.GET.get('so', 'recent')  # 정렬기준
    ...
    # 정렬
    if so == 'recommend':
        question_list = uestion.annotate(num_voter=Count('voter')).order_by('-num_voter', '-create_date')
    elif so == 'popular':
        question_list = uestion.annotate(num_answer=Count('answer')).order_by('-num_answer', '-create_date')
    else:  # recent
        question_list = uestion.order_by('-create_date')
